...
public class PetStore {
    ...
    public void displayAllPetsWithAttributes(boolean friendly, boolean needsAttention, double price) {
        ...
        for (ArrayList<Pet> listOfPets : allPets) {
            displayOneSpeciesWithAttributes(listOfPets,friendly,needsAttention,price);
        }
    }

    //EFFECTS: prints out all pets of this species matching given attributes
    private void displayOneSpeciesWithAttributes(List<Pet> petList, boolean friendly, boolean needsAttention, double price) {
        for (Pet p : petList) {
            boolean a = p.isFriendly() == friendly;
            boolean b = p.needsAttention() == needsAttention;
            boolean c = p.getPrice() <= price;
            if (a && b && c) {
